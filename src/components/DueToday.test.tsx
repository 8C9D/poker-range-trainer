...
describe('DueToday', () => {
  it('shows an empty state when nothing is due', () => {
    render(<DueToday dueRanges={[]} onPractice={vi.fn()} onClose={vi.fn()} />)
  ...
  it('calls onPractice with the chosen range', async () => {
    ...
    render(<DueToday dueRanges={[range]} onPractice={onPractice} onClose={vi.fn()} />)
  ...
  it('calls onClose from "Back to library"', async () => {
    const user = userEvent.setup()
    const onClose = vi.fn()
    render(<DueToday dueRanges={[]} onPractice={vi.fn()} onClose={onClose} />)
