...
describe('MultiActionEditor', () => {
  it('renders the action palette and the grid', () => {
    ...
    expect(screen.getByRole('button', { name: 'AA' })).toBeInTheDocument() // grid cell
  })
  ...
  it('shows per-action percentages for actions in use', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'AA' })) // raise
    await user.click(screen.getByRole('button', { name: 'KK' })) // raise
