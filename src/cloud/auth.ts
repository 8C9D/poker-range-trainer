...
export async function signIn(
  ...
  const { data, error } = await requireClient(client).auth.signInWithPassword({
    email,
    password,
  })
  if (error) throw error
...
/**
 * Subscribe to auth-state changes. Returns an unsubscribe function. When cloud
 * is unconfigured this is a no-op (returns a function that does nothing).
 */
