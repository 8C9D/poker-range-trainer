...
/**
 * Combo explorer (M6): type a hand class (e.g. "AKs") to see its concrete 2-card combos, and
 * optionally type dead/board cards to see how many survive the blockers. All combo math reuses
 * `@core/domain/combos` (via the `enumerateCombos` helper); this screen only gathers the text
 * inputs and renders the combos, dimming the ones the dead cards remove.
 */
      ...
      {error ? (
        ...
        <>
          <Text testID="combo-count" style={styles.count}>
            {hasDead
              ? `${result.survivingCount} of ${result.total} combos`
              : `${result.total} combos`}
          </Text>
