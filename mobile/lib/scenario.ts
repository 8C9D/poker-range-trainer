...
// Drill scenario/verb/feedback copy — the mobile port of the web app's
// src/practice/scenario.ts (web-only, not part of @core). Pure string helpers.

/**
 * The drill's answer button labels: the range's action verb vs Fold. A range without an
 * action type falls back to the generic "In range" verb. The labels never change
 * mid-session, so the buttons stay fixed in place and meaning.
 */
...
/**
 * A one-line scenario context for drill prompts, built from the range's metadata,
 * e.g. "You are UTG, 100bb. First to act — open or fold." Returns null when there is no
...
/**
 * Explanatory feedback for a scored answer: misses explain what the hand actually is and
 * what to do about it; hits confirm briefly.
 */
export function feedbackLine(
  ...
  if (correct) {
    return expectedInRange
      ? `Correct — ${verbs.yes.toLowerCase()} ${hand}.`
      : `Correct — ${hand} is a fold.`;
  }
  return expectedInRange
