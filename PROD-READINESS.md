...
| id | was | sev | fix as landed | status | resolved by |
| --- | --- | --- | --- | --- | --- |
| P2-4 | P2, documented not fixed | P2 | `src/storage/backup.ts:375-391` gives each rollback write its own `try`/`catch`, so the loop reaches every slice however many of them refuse, and `:393` still raises the error that stopped the restore rather than one raised while rewinding (anchors re-based three times: they read `:288-298` and `:299` at `227e3e1`, `:296-306` and `:307` at `f4addad`, `:350-366` and `:368` at `2d09b26`, `:365-381` and `:383` at `b3ed6d9`, and round 7's R6-1 comment fix moved them once more) | RESOLVED | `d13fd15` |
...
The loop has a second operation: `src/storage/backup.ts:377` calls `removeItem` for any slice whose snapshot is `null`, which is every slice on a first-ever import.
...
| id | what it was | sev | fix as landed | status | resolved by |
| --- | --- | --- | --- | --- | --- |
| R3-5 | a rollback write that refuses is caught and discarded, and nothing anywhere records that the library now holds one slice from a different point in time | P2 | `src/storage/backup.ts:245-284` adds an injected reporter seam and `:374-392` reports the damaged slices before raising the restore error; `mobile/platform/crashReporting.ts:63` wires it, `:135` sends it to Sentry as a message carrying key names only | RESOLVED | `6186581` |
...
The trade is one either way, and under-reporting still wins it, because the falsely named slice would be the only entry in the report and a report that fires when the library is intact is worth less than no report at all.
`src/storage/backup.ts:329-356` now says this where the counter is defined, and `:308-310` narrows the doc comment that promised every damaged slice is handed over.
...
| id | what it was | sev | fix as landed | status | resolved by |
| --- | --- | --- | --- | --- | --- |
| R5-1 | `6186581` wrote "a `setItem` that throws is not credited with a replacement it did not make" into `@core` one commit after `5b272e3` recorded that exact step as unsettled | P2 | `src/storage/backup.ts:329-356` states `replaced` as a lower bound, names `getAllKeys` as the step that can make it one short, bounds the gap and gives the reason under-reporting is the chosen direction; `:308-310` narrows the doc comment; the R3-5 paragraph above says the same. **Its justifying sentence named the wrong alternative and was corrected in round 7 - see R6-1** | RESOLVED | `8160e41` |
...
All four probe files were deleted and none is in the tree.
...
| P2-4 | persistence | P2 | `src/storage/backup.ts:244-250` (read against `21f568b`, where those lines were the `catch` block; `d13fd15` rewrote it and the loop is now `:375-391` — re-based per REVIEW-R3 R3-2 and again in rounds 5, 6 and 7) | wrap the rollback loop so a rollback failure cannot replace the original error | restore path only — **RESOLVED in round 3, `d13fd15`** |
...
What closed it: `src/storage/backup.ts:267` `setRestoreDamageReporter` takes an injected reporter, `:374-391` collects the keys whose rewind refused AND whose forward write had already landed, and `:392` hands that set over before the original error is raised; `mobile/platform/crashReporting.ts:63` wires it to `reportRestoreDamage` inside `initCrashReporting`, next to the storage-loss wiring and equally ungated.
...
`src/storage/backup.ts:300-305` (read against `f4addad`, the last commit before R3-5's fix; at HEAD that inner `catch` is `:379-390`, dated rather than rewritten, per REVIEW-R5 R5-2) catches and discards a rewind that refuses, and not raising is correct — raising is the defect P2-4 removed, and the caller is about to be told the actionable first cause.
