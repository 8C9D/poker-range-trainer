...
`mobile/platform/localStorageShim.ts:53` calls `createMMKV(...)`; **before the fix it read `createMMKV({ id: 'poker-range-trainer' })` at line 34, with no `recoveryStrategy`** (anchors re-based per REVIEW-1 F4 — the fix inserted eleven comment lines above the call site, so the pre-fix line numbers no longer resolve; read them against `4551454`, the last commit before the fix — and re-based again per REVIEW-R6 R6-5, since round 2's `beedb9a` moved the call from `:45` to `:53`). Traced through the installed package and the vendored core, every step read from source:
...
| N-2 | NEXT ROUND | P2 | `mobile/platform/storeIntegrity.ts` keeps a key inventory in a second MMKV instance; `localStorageShim.ts:54` checks it as the store opens and `:65` re-records it after every mutation; `components/StorageLossNotice.tsx` tells the user on Today (`app/(tabs)/index.tsx:152`); `crashReporting.ts:115` tells Sentry (it read `:108` until round 5's `6186581` added the restore-damage wiring above it; re-based per REVIEW-R6 R6-5) | RESOLVED | `beedb9a` |
...
| id | was | sev | fix as landed | status | resolved by |
| --- | --- | --- | --- | --- | --- |
| P2-4 | P2, documented not fixed | P2 | `src/storage/backup.ts:396-412` gives each rollback write its own `try`/`catch`, so the loop reaches every slice however many of them refuse, and `:414` still raises the error that stopped the restore rather than one raised while rewinding (anchors re-based six times: they read `:288-298` and `:299` at `227e3e1`, `:296-306` and `:307` at `f4addad`, `:350-366` and `:368` at `2d09b26`, `:365-381` and `:383` at `b3ed6d9`, `:375-391` and `:393` at `3b6beb4`, `:382-398` and `:400` after round 8's R7-1/R7-2 comment fix, and round 9's R8-2/R8-3 fix moved them once more) | RESOLVED | `d13fd15` |
...
The loop has a second operation: `src/storage/backup.ts:398` calls `removeItem` for any slice whose snapshot is `null`, which is every slice on a first-ever import.
...
| id | what it was | sev | fix as landed | status | resolved by |
| --- | --- | --- | --- | --- | --- |
| R3-5 | a rollback write that refuses is caught and discarded, and nothing anywhere records that the library now holds one slice from a different point in time | P2 | `src/storage/backup.ts:245-284` adds an injected reporter seam and `:395-413` reports the damaged slices before raising the restore error; `mobile/platform/crashReporting.ts:63` wires it, `:135` sends it to Sentry as a message carrying key names only | RESOLVED | `6186581` |
...
**The bundler half was an inference sitting inside that correction, and running it showed the stated mechanism was wrong - corrected per REVIEW-R6 R6-2.**
It read "the web root's `package.json` declares no `react-native`, `expo`, `@sentry/*` or `mmkv` dependency, so the bundler would have failed had the compiler not", which was never observed because `tsc -b` stops the script first.
Run directly with the same mutant in place, `npx vite build` does fail - exit 1, `Build failed with 7 errors` - but every one of them is `[PARSE_ERROR] Flow is not supported` inside `mobile/node_modules/react-native/` (`index.js`, `Libraries/NativeComponent/NativeComponentRegistry.js`, `Libraries/Utilities/PolyfillFunctions.js`, `Libraries/Core/Devtools/parseErrorStack.js`, `Libraries/Core/Devtools/getDevServer.js`), and there is no resolution error at all.
Rolldown resolves from the importing file, so `@sentry/react-native` and `react-native` come out of `mobile/node_modules/` whatever the web root declares; what stops it is React Native's own Flow syntax, reached only because the unused import is followed rather than elided.
That observation is conditional on `mobile/node_modules` being installed, which is this tree and any clone that has run the mobile install; in a web-only clone the same mutant fails at resolution instead. Either way the bundler refuses, which is the half that was right.
...
**Both halves were walked in round 7 and the second is tighter than that - see REVIEW-R6 R6-1.** A rewind that refuses without landing means `mmkv.set`, so it needs a slice that HELD something before the restore: where the snapshot is `null` the rewind is `removeItem`, whose only throw candidate on the tracked tree is the same post-write `getAllKeys`, which by then has already taken the value away. A slice that was empty before cannot cost a report at all.
**"On the tracked tree" is load-bearing, and round 7 wrote it here and not into the comment it added in the same commit - corrected per REVIEW-R7 R7-2.** `98d698a` put the unhedged form into `src/storage/backup.ts`, where a reader is the only check there is; the hedge is now in both. What the tracked tree supports is that `mmkv.remove` raises nothing at the react-native-mmkv layer (`mobile/node_modules/react-native-mmkv/cpp/HybridMMKV.cpp:182-189` contains no `throw`) and that its published contract documents `@throws` twice on `set` (`lib/specs/MMKV.nitro.d.ts:39-45`) and none on `remove` (`:74-78`). Below that is `instance->removeValueForKey`, which is MMKVCore under ASSUMPTION 3.
**That verified the two ends of the chain and skipped its middle - opened per REVIEW-R8 R8-2, and the bound holds.**
Between the JS call and that C++ method sit two links neither round opened. The JS half adds nothing: `lib/index.js` exports `createMMKV`, the exists/delete helpers and the hooks, and `lib/createMMKV/createMMKV.js:7-33` returns `factory.createMMKV(config)` - the C++ HybridObject itself - so no JS wrapper stands between `mmkv.remove(key)` and native.
The generated bridge adds nothing either: `mobile/node_modules/react-native-mmkv/nitrogen/generated/shared/c++/HybridMMKVSpec.cpp` is 42 lines of `registerHybridMethod` calls with no `throw` in it.
The link that does hold throws is the one no document named: `react-native-nitro-modules@0.36.5`, `cpp/core/HybridFunction.hpp:90-134`, which is what `registerHybridMethod` builds. Its throw sites are an arity check (`:103-113`), which for `remove(key: string)` fires only on a call passing other than one argument; four `this`/`NativeState` checks (`:195-235`), every one inside `#ifdef NITRO_DEBUG`, which `cpp/utils/NitroDefines.hpp:15-20` leaves undefined whenever `NDEBUG` is set, i.e. in the build that ships; an argument conversion, `JSIConverter<std::string>::fromJSI` = `arg.asString(runtime).utf8(runtime)` (`cpp/jsi/JSIConverter.hpp:156-159`), which throws only for a non-string; and a catch-all (`:119-129`) that RE-RAISES whatever comes from below as a `jsi::JSError`.
The shim calls it as a method on the instance with one string (`mobile/platform/localStorageShim.ts:80-84`), so no call-shape throw can fire. The bound survives at all three links, and the ASSUMPTION 3 caveat gets sharper rather than weaker: the bridge absorbs nothing, so anything MMKVCore raised would arrive in JS as an error.
**Under-reporting is the deliberate direction, and the alternative it beats was misnamed - corrected per REVIEW-R6 R6-1.** It read "counting optimistically would name slices the forward write never reached, which on a full device is most of them", which is the cost of dropping the `index < replaced` guard, not of counting before the write.
Counting before the write leaves `replaced` at the failing index plus one, so the guard still excludes every slice past the failure: the optimistic count can name exactly ONE intact slice, against the one damaged slice the real count can miss.
The trade is one either way, and under-reporting still wins it.
**The sentence that said so leaned on a second absolute, and that one is false as written - corrected per REVIEW-R7 R7-1.**
It read "the falsely named slice would be the only entry in the report", which holds only where no EARLIER rewind refused: the guard admits every index below the failing one, so a slice that really was replaced and really did refuse its rewind lands in the same report as the falsely named one.
Measured rather than argued, with the alternative counter actually in the tree: with `replaced += 1` moved above the write, a forward failure on slice 3 whose rewind refused alongside slice 0's reports `["poker-range-trainer.saved-ranges.v1", "poker-range-trainer.action-accuracy.v1"]`, against `["poker-range-trainer.saved-ranges.v1"]` from the counter as it ships.
The conclusion is unchanged and rests on the case that decides it - where nothing earlier refused, the false name is the whole report and the report fires on an intact library; where something earlier did refuse, the report is already true and the false name adds a key rather than being one.
`src/storage/backup.ts:329-377` now says this where the counter is defined, and `:308-310` narrows the doc comment that promised every damaged slice is handed over.
**That priced one of the two errors and never priced the other, whose own empty-otherwise case is SILENCE - measured per REVIEW-R8 R8-3, and the choice is unchanged.**
`reportRestoreDamage` returns early on an empty list (`src/storage/backup.ts:278`), so when the only damaged slice is the uncounted one, there is no report at all rather than a report short by a key.
Two temporary tests, both deleted: the forward write of `trainingGoal` (the last slice) LANDS and then throws - the iOS shape this is about, `mmkv.set` succeeding and the `getAllKeys` bookkeeping after it refusing - and its rewind then refuses without landing. The restore raises `QuotaExceededError`, `saved-ranges` is rewound to `original`, `trainingGoal` holds the restored file's 50, and the reporter is NOT CALLED. The control in the same file, with `saved-ranges`' rewind refusing too, reports `["poker-range-trainer.saved-ranges.v1"]` while both slices hold restored data.
So the two one-slice errors are symmetric in shape - each is the whole of the report in its own empty-otherwise case - and what decides between them is which needs the rarer failure: the false name needs a forward write that simply refused, the ordinary full-device failure; the missing name needs one that landed and then threw, which on the tracked tree is only the bookkeeping. The comment now states both costs and that asymmetry.
Four rounds have now rewritten this one paragraph - R5-1 an unproven premise, R6-1 a wrong magnitude, R7-1 an unstated condition, R8-3 an unstated cost - which is what a design decision written only in a comment attracts.
...
The sixth is `:300-305` in R3-5's own NEXT ROUND entry, which the same fix moved to the inner `catch` at `:369-380` (`:379-390` after round 7's comment fix, `:386-397` after round 8's) and which is now dated the way round 2's preserved P2-4 paragraph dates its anchors.
...
| id | what it was | sev | fix as landed | status | resolved by |
| --- | --- | --- | --- | --- | --- |
| R5-1 | `6186581` wrote "a `setItem` that throws is not credited with a replacement it did not make" into `@core` one commit after `5b272e3` recorded that exact step as unsettled | P2 | `src/storage/backup.ts:329-377` states `replaced` as a lower bound, names `getAllKeys` as the step that can make it one short, bounds the gap and gives the reason under-reporting is the chosen direction; `:308-310` narrows the doc comment; the R3-5 paragraph above says the same. **Its justifying sentence named the wrong alternative and was corrected in round 7 - see R6-1; the replacement carried an absolute of its own and was corrected again in round 8 - see R7-1** | RESOLVED | `8160e41` |
...
**That was one export per condition, and round 7 re-measured it because one pair cannot separate an edge from compiler noise - per REVIEW-R6 R6-3.**
Seven more exports, all `expo export --platform ios --output-dir dist --clear` with `EXPO_PUBLIC_SENTRY_DSN='https://key@o0.ingest.sentry.io/0'` - the DSN is recorded because its length is in the bundle, which is why these absolute sizes run 7 bytes under the ones above.
...
Run-to-run spread is 3 bytes with the edge and 1 without, so the delta is bounded at 51 to 55 bytes and the 52 above sits inside it.
**"Bounded" is the wrong word for what those seven exports are - corrected per REVIEW-R7 R7-4.** 51 to 55 is the observed min-max of four samples against three, on one machine in one session, of output the line below records as not reproducible: it is the range that was seen, not a limit that was proved. It still carries the only thing the number was ever load-bearing for - tens of bytes, not hundreds.
**And Hermes output here is NOT byte-reproducible**: two exports of the identical tree under identical conditions gave 5,494,545 and 5,494,546 bytes with different SHA-256s (`419c2233…`, `cb2a4c21…`), while Metro's own content hash - the `entry-<hash>.hbc` name, over the JS Metro emits - was identical across every run within a condition.
The JS is deterministic and the bytecode is not, so any future single-pair bundle comparison under about 5 bytes is noise.

**The edge costs tens of bytes, not hundreds, and pulls in no module that was not already shipping.**
...
**That check and the storage-key claim above are round 6's, and round 7 re-measured the table between them without re-running either - re-run per REVIEW-R7 R7-4, and the second one needs a condition it does not state.**
Four exports at `3b6beb4`, all `expo export --platform ios --output-dir dist --clear`: DSN set (5,494,545 bytes, `entry-25d3c8b617dafd746aa9df9931586254.hbc`), DSN set to the EMPTY STRING (5,494,461, `entry-eb0a7c7fe8dde58fd9d1cc8a066752fc.hbc`), the edge removed with the DSN set (5,494,493, `entry-64ee6db602ebed41a98f9db1037d1f8e.hbc`), and the variable UNSET entirely (5,493,421, `entry-5f0e8b0223c403bbf100bdd7056356f2.hbc`).
The storage-key claim reproduces exactly - all nine keys are in the edge-removed bundle - and so does the `strings` result, for the unset case.
It does NOT hold when the variable is present and empty: `getSentryDsn` (`mobile/platform/crashReporting.ts:24-29`) treats that as disabled, so the app is still inert at runtime, but both message literals are in the shipped bytecode and the bundle is 1,040 bytes larger than the unset one.
"Not in the binary at all" is therefore a claim about the variable being ABSENT, not about crash reporting being off, and an EAS env var set to an empty value is the ordinary way to produce the other case.
...
The three explanations that were not eliminated are ruled out rather than argued away: no source map is written at all (`expo export --source-maps` defaults to `false`, and a completed export holds zero `.map` files), and module order and cache keys can only reach the name through that one md5 input, which was reproduced.
Two corrections come with it. The emit the hash is taken over is the UNMINIFIED one, which no build ships, so "over the JS Metro emits" above is too broad. And the name is a fingerprint of the tree AND the condition, not the tree alone: a 43-character DSN on the same tree gives `entry-4d2000cc94bf93321543fb098d335e6e.hbc`, while re-exporting the unchanged tree reproduces the name exactly and produces a different `.hbc` md5 (`d2f4ccc5…` against `d0c6d156…`) - a stable name over non-reproducible bytecode, in one pair of runs.

**The numbers either side of that sentence are attributed now, and 1,040 is not what its position implies - per REVIEW-R8 R8-4.**
It is the whole unset-to-empty delta, not the reporters' cost. In the unminified JS above, BOTH reporter message literals are present in all three conditions - DSN set (9,332,719 bytes), empty (9,332,686) and unset (9,332,710) - so what the variable changes is what minification and Hermes fold away downstream, not what the source carries into the bundle.
The 84-byte gap between the set and empty bundles is 33 bytes of DSN and a 51-byte residue: the set-to-empty difference in that JS is exactly 33 bytes, the length of the placeholder, and a 43-character DSN gives 5,494,556 bytes against the 33-character one's 5,494,545, +11 for +10 characters. The residue is a bytecode-level effect and nothing measured here attributes it.

**And the EAS half of the sentence above was asserted without consulting anything - narrowed per REVIEW-R8 R8-5.**
What the exports show is that a variable PRESENT AND EMPTY produces the second case; how such a value would reach an EAS build is not something this tree answers.
`mobile/eas.json` is 24 lines with no `env` block anywhere (REVIEW-R8 R8-5 says 20, which is wrong and is corrected here) - a `cli` block, three build profiles (`production` carrying only `autoIncrement`) and one `submit` profile - `LAUNCH-CHECKLIST.md:66` asks only for `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` and `SENTRY_PROJECT` as EAS secrets, and the only tracked mention of the DSN outside comments is `mobile/platform/crashReporting.ts:25` and the test that sets it.
Settling the EAS half would take reading that project's own environment variables, which is remote and outside this run - the same boundary FR-1 sits behind.
...
Two temporary probe files using `rangeRemoval`'s `pendingUndo` (`src/storage/rangeRemoval.ts:127`), each asserting the holder is empty before filling it: both pass under the shipped config, and under `--no-isolate --no-file-parallelism` the second fails at the empty assertion, which is what makes the probe evidence rather than decoration.
**That was a stand-in, and it was called "the same shape as `reportDamage`, module state with no reset", which is the one thing it is not - corrected per REVIEW-R6 R6-4.** `pendingUndo` has `clearDeletedRanges` (`src/storage/rangeRemoval.ts:143-145`) and a public reader `peekDeletedRanges` (`:138-140`); `setRestoreDamageReporter` has a setter and nothing else, which is exactly why the question was worth asking.
**Redone in round 7 on the real holder.** Two probe files importing `setRestoreDamageReporter` from `src/storage/backup.ts`, each forcing a damaging restore in the `src/storage/backup.test.ts:462` shape with no reporter installed in its own registry, asserting nothing was reported, then installing one and re-running to prove the mechanism fires; the observable is a file in `os.tmpdir()`, because a shared observable is what module isolation would otherwise hide.
Under `--no-file-parallelism` both pass (2 files / 2 tests); under `--no-file-parallelism --no-isolate` the second fails at its first assertion, `expected 'aa' to be 'a'` - probe a's reporter firing inside probe b's restore.
So the conclusion round 6 reached is right, and now it is evidence about the holder it is a claim about.
**The PASS half of that was measured under a flag the shipped config does not set - re-run per REVIEW-R7 R7-3.**
`vitest.config.ts` sets neither `isolate` nor `fileParallelism` and `test:run` (`package.json:11`) is a plain `vitest run`; the flag was there because the observable was a shared file in `os.tmpdir()`, which two files running at once would race on, and it then sat under the claim as well as under the control.
It is orthogonal to isolation, from the runner's own source: `node_modules/vitest/dist/chunks/coverage.DM_a_rWm.js:223-225` is the only non-browser read of `fileParallelism` and it sets `maxWorkers = 1` and nothing else, while two files sharing a process is gated on `isolate === false` alone (`node_modules/vitest/dist/chunks/cli-api.BfdDOPPI.js:3503` consults the shared runners only then, `:3460` fills them only then, and `:3565` throws `Isolated tasks should not share runners` otherwise). `node_modules/vitest/dist/chunks/defaults.9aQKnqFk.js:47` defaults `isolate: true`.
**The one branch that reading skipped is now read forwards, and the package it was faulted for not grepping is not installed here - per REVIEW-R8 R8-7.**
`cli-api.BfdDOPPI.js:3801` routes `isolate === true && groupOrder === 0 && maxWorkers === 1` into the `sequential` group, and what it pushes is `[spec]`, a one-element array; `:3831` appends that group; `:3624-3675` turns each element of a group's `specs` into ONE task whose `context.files` is that array, run by a single `pool.run(task, method)` at `:3684`.
So a spec in the sequential group is its own task carrying one file - read from the branch itself rather than from `:3813`'s converse, and `:3813` remains the only place several specs share one array.
`tinypool` is not a dependency of `vitest@4.1.8` and is not installed in this tree (`npm ls tinypool` is empty); the only occurrence of the string in its `dist` is a GitHub URL in a comment at `cli-api.BfdDOPPI.js:3097`. Worker lifetime is owned by vitest's own fork pool, in the chunk already being read.
Re-run with an observable that survives the shipped config: `process.env.PRT_PROBE_SINK`, pointed by each probe at its own path on load and read by the reporter at call time, so it is shared exactly when the two files share a process and private when they do not.
Under the shipped config, no flags: 2 files / 2 tests pass, in pids 40061 and 40060. Under `--no-isolate --no-file-parallelism`: one pid, 40444, and probe b fails at `expect(existsSync(SINK)).toBe(false)`.
Under `--no-isolate` alone the pair passes in two different pids, which is exactly why the flag belongs on the control and not on the claim: with parallelism as shipped, the scheduler can put the two files in different workers and nothing leaks.
All six probe files were deleted and none is in the tree.
**Each of those rows is one run, and re-running them 22 times moves what each one proves - per REVIEW-R8 R8-6.**
Two probes of the same design were rebuilt at `464f556`, each also recording its pid and the real-time order, and run across four configurations.
...
| `--no-isolate --no-file-parallelism` | 8 | probe b failed in 2 of 8 | one pid every time | a before b in exactly those 2 |

The pids are not the weaker half of the evidence; under the shipped config they are the load-bearing half.
A module-level holder cannot cross a process boundary, so two distinct pids are the reason nothing leaked, and probe b's sink assertion can only discriminate in a run where probe a has already installed its reporter - which was 1 of the 5 shipped runs. The other four are consistent with isolation without testing it.
The negative control fires exactly when the scheduler runs a before b, and Vitest fixes no such order: 2 of 8 runs had it and both failed, the other 6 ran b first and passed in one process with a shared registry. Rounds 7 and 8 each ran the control once and each drew the failing order.
And `--no-file-parallelism` alone puts each file in its OWN process even at `maxWorkers: 1`, so isolation in the forks pool is process separation rather than a fresh registry inside one process - a stronger guarantee than the claim needs, and the reason no shipped-config run can leak.
...
Round 7's own comment fix then moved the same set once more, and the current values are in the rows above.
...
**Round 7 closed that bound and it was holding four wrong anchors - see REVIEW-R6 R6-5 and the round 7 section below.**
...
| id | what it was | sev | fix as landed | status | resolved by |
| --- | --- | --- | --- | --- | --- |
| R6-1 | the comment justified under-reporting with the cost of a DIFFERENT alternative - "counting optimistically ... on a full device, most of them" is what dropping the `index < replaced` guard costs, not what counting before the write costs | P2 | `src/storage/backup.ts:353-377` now names the alternative it actually beats, states the trade as one slice each way, and parenthesises the guard-less case as the different and worse thing it is; `:340-351` adds the `removeItem` half of the bound; the R3-5 paragraph above says the same. **Both halves needed narrowing in round 8 - see R7-1 and R7-2** | RESOLVED | `98d698a` |
...
**Two things that sweep left open are now closed - per REVIEW-R7 R7-5.**
The scope it covers is this file and `LAUNCH-CHECKLIST.md`, and it never said what the rest of the repo holds. It holds nothing: `README.md`, `AGENTS.md`, `FEATURE-AUDIT.md`, `TRIM-REPORT.md` and all nine files in `docs/` were opened and carry no `file:line` anchor at all, the only `:NNNN` hits in any of them being `localhost:5173`. `review/findings.md` and `review/targets.md` hold 112 between them, 13 of which already point past the end of the file they name - which settles round 4's call about `review/targets.md:136` and `:144` for the whole of both files rather than for the two lines it was made about, and `docs/archive/` holds 6 more in files the directory name dates.
And the one anomaly the sweep dismissed as a script mis-attribution was a real defect in a sentence here: it is corrected at the P2-5 trace below. A second extraction of this file also returns different totals from the 119 and 120 recorded above (159 fully-qualified occurrences, 130 distinct; 176 bare, 115 distinct), so the count is a property of the script rather than of the file, and resolving each basename against the whole tree rather than a fixed prefix list found no ambiguity.
...
**The convention that makes a bare `:NN` resolvable at all is this file's own, it differs between rounds, and it was written down nowhere - stated here per REVIEW-R8 R8-8, because `PROD-READINESS.md`'s own `HybridMMKV.cpp:135` fix appeals to it.**
It is this: a fully-qualified `path:NN` names its own file, and a bare `` `:NN` `` continues from the last fully-qualified path named before it, carrying across sentences and lines until the next qualified path replaces it.
Round 7's resolver stopped at the line boundary instead and left every bare anchor further down unattributed, which is why the totals the rounds report keep moving. A third extraction at `464f556` returns 385 anchors carrying a line number under the carrying rule, against 313 plus 72 left unattributed under the line-bounded one, over exactly the same text: the count is a property of the extractor, and only the rule makes it mean anything.
Under either rule, nothing in this file or `LAUNCH-CHECKLIST.md` points past the end of the file it names, which is what "all resolve" above is worth. Three anchors are ambiguous by basename alone (`README.md:11-13`, `README.md:16-17`, `package.json:11`); all three are the repo-root files and were opened.
And "re-opened" is the wrong word for reading a resolver's report. What is opened by hand each round is a sample - in round 9, ten, including every anchor its own comment fix moved and the two the P2 table dates to `21f568b`.
...
| P2-4 | persistence | P2 | `src/storage/backup.ts:244-250` (read against `21f568b`, where those lines were the `catch` block; `d13fd15` rewrote it and the loop is now `:396-412` — re-based per REVIEW-R3 R3-2 and again in rounds 5, 6, 7 and 8) | wrap the rollback loop so a rollback failure cannot replace the original error | restore path only — **RESOLVED in round 3, `d13fd15`** |
...
2. **All nine keys share one MMKV instance**, so P0-1's blast radius is the whole library. Evidence: `mobile/platform/localStorageShim.ts:50-57` creates exactly one instance and every shim method routes through `getStore()` (re-based per REVIEW-1 F4, and again per REVIEW-R6 R6-5 — it read `:42-48` until round 2's `beedb9a`).
...
- **No PII reaches Sentry.** Every interpolated `throw` in `src/domain/` carries poker notation or card tokens only (`src/domain/cards.ts:42`, `src/domain/rangeNotation.ts:149`, and eleven siblings); `git grep` for range `notes`/`name` in a thrown message returns nothing. `attachScreenshot`, `attachViewHierarchy` and both replay sample rates are pinned off at `mobile/platform/crashReporting.ts:74-77` (it read `:56-59` at the baseline `21f568b` and had been moved twice since, first by round 2's `beedb9a` to `:67` and then by round 5's `6186581`; re-based per REVIEW-R6 R6-5, first mover named per REVIEW-R7 R7-5).
...
- **Which failure mode a missing org/project actually produces** (added per REVIEW-FINAL FR-1). **Still open after rounds 2 and 3: neither ran an EAS build, so nothing new was observed and the checklist's account of it (`LAUNCH-CHECKLIST.md:197-210`, and `:68-69` since round 3 corrected it) is still inference.** It stays inferred from `sentry-xcode.sh` and the vendored `sentry-cli`, and the first real production build should be used to settle it. The vendored script and CLI both point at a failed build rather than a silent unsymbolicated one, but settling it needs a real `sentry-cli` invocation against sentry.io or an EAS build log — both non-local, both prohibited. P1-1's severity therefore rests on a premise this run could not close: if the true behavior is a failed build, the ledger's own rubric would make it "cannot deploy", which is P0. It is left at P1 rather than raised, because the evidence for the harsher rating is exactly the evidence that cannot be confirmed here, and because a failed build announces itself where an unsymbolicated one does not. The fix is identical under either reading.
  **Settled 2026-08-15, outside this run, by the first real production build.** EAS build `bd69c773` ran with `SENTRY_AUTH_TOKEN`, `SENTRY_ORG` and `SENTRY_PROJECT` set and its Xcode log records the upload succeeding: "Uploaded files to Sentry", org `<sentry-org>`, project `poker-range-trainer`, release `com.arthurzhang.pokerrangetrainer@1.0.0+3`. The missing-variable case itself was still never run, so the exact failure mode of an unset org/project stays unobserved; the loud-failure half of the inference did get direct evidence the same day, from an unrelated failure: build `2355aaeb`'s bundle phase failed (a machine-local `mobile/ios/` had been uploaded; see `LAUNCH-CHECKLIST.md` step 7's record) and `sentry-xcode.sh` surfaced it as an Xcode `error:` plus `exit 1` - a failed build, not a silent skip. P1-1's fix is therefore verified working rather than inferred, and the severity question is moot for this release.
...
What closed it: `src/storage/backup.ts:267` `setRestoreDamageReporter` takes an injected reporter, `:395-412` collects the keys whose rewind refused AND whose forward write had already landed, and `:413` hands that set over before the original error is raised; `mobile/platform/crashReporting.ts:63` wires it to `reportRestoreDamage` inside `initCrashReporting`, next to the storage-loss wiring and equally ungated.
...
`src/storage/backup.ts:300-305` (read against `f4addad`, the last commit before R3-5's fix; at HEAD that inner `catch` is `:400-411`, dated rather than rewritten, per REVIEW-R5 R5-2) catches and discards a rewind that refuses, and not raising is correct — raising is the defect P2-4 removed, and the caller is about to be told the actionable first cause.
