...
- [x] **[Y]** Decide whether the repo needs a LICENSE (only matters if it is or will be public). Closed 2026-08-18: the repo is going public, so **GPL-3.0** was chosen and the full licence text added as `LICENSE` at the repo root. Copyright is held as **`Copyright (C) 2026 8C9D`** - the GitHub handle that owns the repo, not a personal name, because Pass 8b makes anonymity a hard requirement until the Apple membership converts to Organization. **Re-attribute the copyright line to the corporation once incorporation completes** (see `docs/business-chain-guide.md` step 1); the handle is a placeholder for the entity, not the intended long-term holder.
...
- [ ] **[Y]** Screenshots - step 10. Candidate set captured 2026-08-19 by agent: all ten shots (range editor mid-edit, drill question, drill miss + explanation, populated Progress, Today with a 9-day streak) at both required sizes - 6.7" 1290x2796 and 6.5" 1242x2688 - in `screenshots/` at the repo root (PNGs gitignored as regenerable upload assets; the seed data `screenshots/seed-backup.json` is tracked so a re-shoot starts from the same library), Release build of 1.0.0 (5)-equivalent source on iOS 18.5 simulators (the 26.3 runtime has a local emoji-rendering defect; the app is unaffected), seeded with realistic data that passes the app's own backup validation. Remaining: user approves (or re-shoots on device) and uploads to App Store Connect.
