---
name: roadmap-slice
description: Implement the next poker-range-trainer roadmap slice (or a specific one via a slice number), validate it, commit it, and queue the prompt for the slice after it. Use when the user wants to advance docs/roadmap.md one small, committed slice at a time.
---

# roadmap-slice

Advance the roadmap by exactly one slice: read the queued plan, build it, validate
it, commit it, and write the prompt for the next slice so a later invocation can
continue. One slice per invocation.
...
one commit and advances the pointer to the next slice.
...
4. **Re-check the plan.** Skim the relevant roadmap section and the current code for
   ...
   done in the log, regenerate the Next slice prompt, commit that bookkeeping, and
   report. Otherwise turn the queued prompt into a concrete, minimal plan: the exact
   files to create/modify and the tests to add.
...
6. **Validate.** Run, in order:
   ...
   the work uncommitted, and report what failed — do not commit and do not advance
   the pointer.
...
8. **Commit.** Stage the implementation, its tests, and the updated state file, and
   commit them together as one slice commit. Use a conventional message (`feat:`,
   `fix:`, `refactor:`, `docs:` …) describing the slice, ending with:
...
9. **Report.** Summarize: which slice was built, the files changed, the validation
   results (state them honestly), the commit hash, and the number + title of the
   slice now queued for the next invocation.
